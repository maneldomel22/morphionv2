...
import { ChevronLeft, ChevronRight, Upload, X, Flame, Info } from 'lucide-react';
import { PROMPT_LIMITS } from '../../services/wanHotPromptService';
import { translateToEnglish } from '../../services/morphyHotService';
...
export default function HotWanQuiz({ onComplete, onCancel }) {
  ...
  const environmentTimeout = useRef(null);

  useEffect(() => {
    return () => {
      if (actionTimeout.current) clearTimeout(actionTimeout.current);
      if (environmentTimeout.current) clearTimeout(environmentTimeout.current);
    };
  }, []);
  ...
  const handleTextChange = async (field, value, timeoutRef) => {
    ...
    timeoutRef.current = setTimeout(async () => {
      try {
        ...
        setTranslations(prev => ({ ...prev, [field]: translated }));
      } catch (error) {
          ...
          <div className="space-y-4">
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <div className="flex items-start gap-3">
                <Flame size={20} className="text-red-600 flex-shrink-0 mt-0.5" />
                <div className="flex-1">
                  <h4 className="font-semibold text-red-600 mb-1">Modo Influencer 🔥 (18+)</h4>
                  <p className="text-sm text-textSecondary">
                    Este modo gera conteúdo adulto artístico. A imagem de referência será usada apenas para identidade facial.
                    Nunca use imagens de menores de idade.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-red-600 mb-1">Limite de 800 caracteres</h4>
                  <p className="text-sm text-textSecondary">
            ...
            <div>
              ...
              <p className="text-xs text-textSecondary mb-3">
                Digite em português. A tradução para inglês é automática.
              </p>
              <textarea
                value={answers.actionText}
                onChange={(e) => handleTextChange('actionText', e.target.value, actionTimeout)}
                placeholder="Ex: em pé com confiança, caminhando devagar, movimento natural, postura relaxada, sensual mas com bom gosto"
                rows={4}
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => handleTextChange('environmentText', e.target.value, environmentTimeout)}
                placeholder="Ex: praia ensolarada com areia dourada, oceano azul ao fundo, ondas suaves, céu claro, luz do dia quente de verão"
                rows={4}
                className="w-full px-4 py-3 bg-surfaceMuted border border-borderColor rounded-lg text-textPrimary placeholder-textTertiary focus:outline-none focus:ring-2 focus:ring-brandPrimary/50 focus:border-brandPrimary transition-all resize-none"
              />
              <div className="flex justify-between items-center mt-2">
